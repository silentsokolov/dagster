...
from dagster.core.definitions import IPipeline, PipelineDefinition, JobDefinition
...
def execute_run(
    ...
    if isinstance(pipeline, PipelineDefinition):
        if isinstance(pipeline, JobDefinition):
            error = "execute_run requires wrapped job but received job definition directly instead."
        else:
            error = (
                "execute_run requires wrapped pipeline but received pipeline definition "
                "directly instead."
            )
        raise DagsterInvariantViolationError(
            f"{error} To support hand-off to other processes please wrap your definition in "
            "a call to reconstructable()."
        )
